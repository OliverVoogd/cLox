...
static bool callValue(Value callee, int argCount)
{
    if (IS_OBJ(callee))
    {
        ...
        case OBJ_FUNCTION:
            runtimeError("A function object is being called but should be wrapped in a closure.");
        case OBJ_NATIVE:
        {
...
#define BINARY_OP(valueType, op)                        \
    ...
    for (;;)
    {
        ...
        case OP_GET_PROPERTY:
        {
            ...
            {
                // New: return a NIL value
                // push(NIL_VAL);
                return INTERPRET_RUNTIME_ERROR;
            }
            break;
            // Original: runtime error if getting a non set property
            // runtimeError("Undefined property '%s'.", name->chars);
            // return INTERPRET_RUNTIME_ERROR;
        }
